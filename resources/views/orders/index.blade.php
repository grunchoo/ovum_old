    ...
    <div class="widget-content searchable-container list">
        ...
        <div class="row">
            ...
            <div class="col-xl-8 col-lg-7 col-md-7 col-sm-7 text-sm-right text-center layout-spacing align-self-center">
                ...
                @can('orders.create')<a href="{{route('orders.create')}}" class="btn btn-primary">Crear</a> @endcan
                @can('orders.orderlist')<a href="{{route('orders.orderlist')}}" class="ml-1 btn btn-info">Todos</a> @endcan
                @can('orders.selectorder')<a href="{{route('orders.selectorder')}}" class="ml-1 btn btn-secondary">Ordenes x Nacimientos</a>@endcan
                </div>
            </div>
        </div> 
                        ...
                        <div class="user-profile col-md-5"> 
                            ...
                            <div class="user-meta-info">
                                ...
                                <p class="user-work"  data-occupation="" > #{{  $row->id }} - Entrega: {{ Carbon\Carbon::parse($row->fecha_entrega)->format('d/m/Y') }} /
                                    @if ($row->aprovacion =='Pendiente') 
                                    <span class="badge badge-pill outline-badge-warning">Pendiente </span>
                                    @elseif ($row->aprovacion =='Rechazado') 
                                    <span class="text-danger bs-tooltip" title="{{$row ->motrechazo}}">Rechazado </span>
                                    @elseif ($row->aprovacion =='Aprovado') 
